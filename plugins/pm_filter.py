...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if UPDATES_CHANNEL and not await is_subscribed(client, query):
...
async def auto_filter(client, msg, spoll=False):
    ...
    url=await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=files_")
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}",callback_data=f'{pre}#{url}{file.file_id}'
    ...
    if imdb:
        ...
        cap = f"<b>Rᴇǫᴜᴇsᴛᴇᴅ ᴍᴏᴠɪᴇ ɴᴀᴍᴇ :</b> <code>{search}</code>"
